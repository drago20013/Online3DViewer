...
export class Viewer {
  ...
  //let meshesWithColors = [{ meshInstance, highlightColor }]
  //assumptions:
  //if same mesh is in two or more arrays the last color will be used
  //viewer.SetMultipleMeshesHighlight(meshesWithColors)

  SetMultipleMeshesHighlight(meshesToHighlightWithColor) {
    let withPolygonOffset = this.mainModel.HasLinesOrEdges();

    this.mainModel.EnumerateMeshesAndLines((mesh) => {
      meshesToHighlightWithColor.forEach(element => {
        let highlighted = element.meshInstances.GetId().IsEqual(mesh.userData.originalMeshInstance.GetId());
        if (highlighted) {
          if (mesh.userData.threeMaterials === null) {
            mesh.userData.threeMaterials = mesh.material;
            mesh.material = CreateHighlightMaterials(mesh.userData.threeMaterials, element.highlightColor, withPolygonOffset);
          }
          // } else {
          //   if (mesh.userData.threeMaterials !== null) {
          //     mesh.material = mesh.userData.threeMaterials;
